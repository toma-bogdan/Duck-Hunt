...
void Lab2::Init()
{
...

    {
        ...
        meshes["tet_A"] = new Mesh("generated tet 1");
        meshes["tet_A"]->InitFromData(vertices, indices);

        // Actually create the mesh from the data
        CreateMesh("tet_A", vertices, indices);
    }
...
void Lab2::Update(float deltaTimeSeconds)
{
    ...
    RenderMesh(meshes["cube_B"], shaders["VertexColor"], glm::vec3(1.5f, 0.5f, 2), glm::vec3(0.25f));

    // TODO(student): Draw the tetrahedron
    RenderMesh(meshes["tet_A"], shaders["VertexColor"], glm::vec3(-2.5f, 0.5f, -2), glm::vec3(0.25f));
