...
void Lab5::Update(float deltaTimeSeconds)
{
    ...
    {
        glm::mat4 modelMatrix = glm::mat4(1);
        modelMatrix = glm::translate(modelMatrix, glm::vec3(1, 1, -2));
        modelMatrix = glm::scale(modelMatrix, glm::vec3(2, 2, 2));
        RenderMesh(meshes["sphere"], shaders["Simple"], modelMatrix);
    }
    {
        glm::mat4 modelMatrix = glm::mat4(1);
        modelMatrix = glm::translate(modelMatrix, glm::vec3(6, 0.5f, -4));
        modelMatrix = glm::rotate(modelMatrix, RADIANS(100.0f), glm::vec3(5, 5, 5));
        RenderMesh(meshes["box"], shaders["Simple"], modelMatrix);
    }
...
void Lab5::OnMouseMove(int mouseX, int mouseY, int deltaX, int deltaY)
{
    ...
    if (window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
    {
        ...
        if (window->GetSpecialKeyState() & GLFW_MOD_CONTROL) {
            ...
            camera->RotateThirdPerson_OY(sensivityOX * -deltaY);
